...
# TODO - See if "protocol" classification of papers caused a problem and I need to change them all to "review"
...
students=load_data("students_grads.csv") %>%
  ...
  mutate(role = paste(role, " (", status, ")", sep=""),
         name=bold(name),
         department = ifelse(is.na(lab), department, paste(department, " (", lab, ")", sep="")), # Add lab if present
...
# TODO: The below function can supposedly format individual citations, maybe giving me more control. But seems unnecesssarily complicated
#  Taken from https://stackoverflow.com/questions/63141295/creating-formatted-references-with-different-citation-styles-to-academic-papers
...
presents = load_data("presentations.csv", ignore="date") %>%
  ...
  mutate(date=as.Date(date), 
         international=ifelse(international, yes="§", no="")) %>%
  arrange(desc(date)) %>%
  ...
  mutate(invited_or_volunteered = ifelse(invited_or_volunteered=="invited", yes="(Invited)", no=""))  # Remove non-invited things
