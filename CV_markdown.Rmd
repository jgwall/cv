...
## Invited Presentations _(* = international)_
...
```{r membership_societies}
# Load and format data
societies = load_data("sources/membership_societies.csv") %>%
  arrange(start, stop ) %>%
  mutate(time=paste(start, ifelse(is.na(stop), yes="Present", no=stop), sep="-")) %>%
  relocate(time)

# Split by current and prior
current = societies %>%
  filter(is.na(stop)) %>%
  select(-start, -stop)
prior = societies %>%
  filter(!is.na(stop) & !(organization %in% current$organization)) %>%
  select(-start, -stop)
...
## TODO: Public Outreach and Service
...
for(myperiod in unique(meetings$period)){
