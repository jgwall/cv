...
# TODO: Want to make a nice table summary of pubs in different time periods and article types
...
# Table out output - TODO: figure out how to remove comment, or use different formatting
#as.matrix(table(pubs$period, pubs$type)) %>% xtable::xtable()
table(pubs$period, pubs$type) %>% 
  as.data.frame.matrix() %>%
  as_tibble(rownames="period") %>% # converting to tibble removes row names
  xtable::xtable()
```

```{r print_pubs, results="asis"}
# Loop over and print out. (Not using group_map to preserve descending year order)
