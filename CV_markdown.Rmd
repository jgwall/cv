...
# TODO: May be able to use kableExtras to format Student Mentorship so that columns align across groups (or I just specify them manually; that's probably simpler)
---

```{r formatting, results="asis", echo=FALSE}
cat("\\renewcommand{\\labelitemi}{\\textbullet}") # Reset bullets to round circles
#cat("\\usepackage{tabularx}") # Include tabularX for variable-width columns
```
...
students=load_data("sources/students.csv") %>% 
  ...
  group_map(function(x, ...){
    cat("###", unique(x$role), "\n")
    x %>% select(-role) %>% baretable(align="llllX")
  }, .keep=TRUE)
```
