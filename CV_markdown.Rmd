...
# TODO: Uncomment updat-pubs section when back in good wifi
...
for(myperiod in unique(presents$period)){
  ...
  presents %>% filter(period==myperiod) %>%
    select(-period) %>%
    baretable(align=c("l", "l", "X"), tabular.environment = "xltabular")
}
```
...
totals %>%
  ...
  kableExtra::kable_styling(latex_options = c("hold_position")) %>%
  add_header_above(c(" ", "Total", "Wallace Lab", "Total", "Wallace Lab")) %>%
  add_header_above(c(" ", "PI"=2, "Co-PI"=2), bold=TRUE) %>%
  column_spec(1, bold = T) %>%
  row_spec(nrow(totals), hline_after=TRUE) %>% 
  row_spec(nrow(totals)+1, italic=TRUE, background="white") # Change format for "totals" row
...
format_grants_full=function(x){
  x = x %>% mutate(mine_desc = ifelse(is.na(amount_wallace), yes="",
                              no=paste(" (", source, "; ",
  ...
  x %>%
    mutate(role=role,
           ...
           description = paste(bold(title), mine_desc,". ", 
                               ...
                               month_year(stop), ".",
              sep=""), .keep="none") %>%
    baretable(align=c("l", "l", "l", "X"), tabular.environment="xltabular")
}
...
# TODO: Professional Development

# TODO: Academic Service

## TODO: Service to the University

## TODO: Service to the Scientific Community

## TODO: Public Outreach and Service

# Other

## TODO: Meetings attendede
